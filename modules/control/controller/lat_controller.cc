...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  ...
  matrix_a_(2, 3) = 1.0;
  matrix_a_(3, 2) = (c_f*l_f-c_r*l_r)/i_z;

  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);
  matrix_a_coeff_(1, 1) = -1.0*(c_f+c_r)/mass);
  matrix_a_coeff_(1, 3) = (c_r*l_r-c_f*l_f)/mass;
  matrix_a_coeff_(3, 1) = (c_f*l_f-c_r*l_r)/i_z;
  matrix_a_coeff_(3, 3) = -1.0*(c_f*l_f^2+c_r*l_r^2)/i_z;
